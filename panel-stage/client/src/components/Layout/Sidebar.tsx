  ...
  ({
    ...
    return (
      <Box key={subItem.id}>
        ...
        >
          <Box
            sx={{ display: "flex", alignItems: "center", p: 0.75, gap: 0.75 }}
          >
            <Box
              sx={{
                width: 24,
            ...
            >
              {subItem.subItems
                ...
                .map((nestedItem: any) => {
                  ...
                  const NestedIcon: any =
                    IconMap[nestedItem.icon] || IconMap.Help;
...
export default function Sidebar({
  ...
  const { user: authUser, clearAuth, permissions } = useAuthStore();
  ...
  const filteredMenuItems = useMemo(() => {
    const permissionFilteredItems = filterMenuItemsByPermission(
      menuItems,
      permissions,
    );
  ...
  }, [searchTerm, menuItems, permissions]);
        ...
        >
          <Box
            sx={{
              ...
              flexShrink: 0,
              overflow: "hidden",
              animation: tenantSettings?.logoUrl
                ? "none"
      ...
      <Box sx={{ px: 1, pt: 0.5, pb: 0.5, position: "relative", zIndex: 1 }}>
        <Button
          ...
          onClick={(e: React.MouseEvent<HTMLButtonElement>) =>
            setQuickMenuAnchor(e.currentTarget)
          }
          sx={{
                ...
                >
                  ...
                  <Box>
                    <Typography
                      variant="body2"
                      fontWeight={600}
                      sx={{ color: "#1E293B", fontSize: "0.8rem" }}
                    >
                      {item.label}
                    </Typography>
                    <Typography
      ...
      >
        {filteredMenuItems.length === 0 && searchTerm ? (
          ...
          filteredMenuItems.map((item, index) => {
            ...
            const prevSection =
              index > 0 ? filteredMenuItems[index - 1].section : null;
            const shouldShowHeader =
              showSectionHeader && item.section !== prevSection;
