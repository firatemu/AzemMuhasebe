...
export default function IrsaliyePrintPage() {
    ...
    const fetchIrsaliye = async () => {
        try {
            ...
            setIrsaliye({
                ...data,
                kalemler: (data.kalemler || []).map((k: { discountRate?: number; iskontoOran?: number; [key: string]: unknown }) => ({
                    ...k,
                    iskontoOran: Number(k.discountRate ?? k.iskontoOran ?? 0),
                })),
            });
        } catch (error) {
                    ...
                    <Table size="small" sx={{ '& td, & th': { fontSize: 'inherit', border: 'none', py: 1 } }}>
                        <TableHead>
                            <TableRow sx={{ borderBottom: '2px solid #06b6d4' }}>
                                ...
                                <TableCell align="center" sx={{ fontWeight: 'bold', color: '#06b6d4' }}>MALZ. İND. %</TableCell>
