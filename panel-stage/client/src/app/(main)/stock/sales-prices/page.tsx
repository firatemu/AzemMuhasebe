...
export default function SatisFiyatlariPage() {
  ...
  const fetchStoklar = async () => {
    ...
    try {
      ...
      const mappedItems: Stok[] = items.map((product: any) => ({
        id: product.id,
        productId: product.id,
        stokKodu: product.code || product.stokKodu || '',
        stokAdi: product.name || product.stokAdi || '',
        marka: product.brand || null,
        satisFiyati: product.salePrice || product.satisFiyati || 0,
        alisFiyati: product.purchasePrice || product.alisFiyati || undefined,
        priceType: 'SALE',
      ...
      setSnackbar({ open: true, severity: 'error', message: 'Ürünler yüklenirken hata oluştu.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSavePrice = async (stok: Stok) => {
    ...
    try {
      ...
      await axios.post('/price-cards', {
        ...
        notes: `Hızlı güncelleme • ${now.toLocaleString('tr-TR')}`,
      });
