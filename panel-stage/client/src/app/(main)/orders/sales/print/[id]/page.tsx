...
export default function SiparişPrintPage() {
  ...
  const fetchSipariş = async () => {
    try {
      ...
      const items = data.items || data.kalemler || [];
      setSipariş({
        id: data.id,
        siparisNo: data.orderNo || data.siparisNo,
        siparisTipi: data.orderType || data.siparisTipi || '',
        durum: data.status || data.durum || '',
        tarih: data.date || data.tarih,
        vadeTarihi: data.dueDate || data.vadeTarihi || data.vade,
        toplamTutar: Number(data.subtotal ?? data.totalAmount ?? data.toplamTutar ?? 0),
        ...
        kalemler: items.map((item: Record<string, unknown>) => ({
          id: item.id as string,
          stokId: (item.productId || item.stokId) as string,
          ...
          tutar: Number(item.amount ?? item.tutar ?? (Number(item.totalAmount ?? 0) - Number(item.vatAmount ?? 0))),
          kdvTutar: Number(item.vatAmount ?? item.kdvTutar ?? 0),
          stok: {
            stokKodu: (item.product as { code?: string })?.code || (item.stok as { stokKodu?: string })?.stokKodu || '',
            stokAdi: (item.product as { name?: string })?.name || (item.stok as { stokAdi?: string })?.stokAdi || '',
            birim: (item.product as { unit?: string })?.unit || (item.stok as { birim?: string })?.birim || (item.unit as string) || '',
          },
        })),
      });
    } catch (error) {
        ...
        <Table size="small" sx={{ '& td, & th': { fontSize: 'inherit', py: 0.5 } }}>
          <TableHead>
            <TableRow sx={{ bgcolor: '#191970' }}>
              ...
              <TableCell align="center" sx={{ color: 'white', fontWeight: 'bold' }}>Malzeme İnd. %</TableCell>
          ...
          <Table size="small" sx={{ '& td, & th': { fontSize: 'inherit', border: 'none', py: 1 } }}>
            <TableHead>
              <TableRow sx={{ borderBottom: '2px solid #667eea' }}>
                ...
                <TableCell align="center" sx={{ fontWeight: 'bold', color: '#667eea' }}>MALZ. İND. %</TableCell>
