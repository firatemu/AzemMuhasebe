...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    if (tenantId) {
      ...
      if (tenant && tenant.status === TenantStatus.PENDING) {
        ...
        console.log(
          `📧 [AuthService] Hoş geldiniz maili gönderiliyor: ${user.email}`,
        );
        this.emailService
          .sendWelcomeEmail(user.email, dto.firstName, dto.lastName)
          .then(() => {
            console.log(
              `✅ [AuthService] Hoş geldiniz maili gönderildi: ${user.email}`,
            );
          })
          .catch((error) => {
            console.error(
              `❌ [AuthService] Hoş geldiniz maili gönderilemedi: ${user.email} - Hata: ${error.message}`,
            );
          });
        ...
        return {
          ...
          message:
            tenant.subscription?.status === SubscriptionStatus.PENDING
  ...
  async login(dto: LoginDto) {
    try {
      // Kullanıcıyı bul — email veya kullanıcı adı ile giriş
      ...
      if (!isSuperAdmin && user.tenantId) {
        const hasBaseLicense = await this.licenseService.hasBasePlanLicense(
          user.id,
        );
        if (!hasBaseLicense) {
          throw new UnauthorizedException(
            'Ana paket lisansınız bulunmuyor. Lütfen yöneticinizle iletişime geçin.',
          );
        }
      }
      ...
      if (!isAdminUser && user.tenantId && user.tenant) {
        ...
        if (user.tenant.status === TenantStatus.PENDING) {
          ...
          if (subscription?.status === SubscriptionStatus.PENDING) {
            ...
            const isTrialPlan =
              plan && (plan.slug === 'trial' || Number(plan.price) === 0);
        ...
        if (user.tenant.subscription) {
          ...
          if (subscription.status === SubscriptionStatus.CANCELED) {
            throw new UnauthorizedException(
              'Aboneliğiniz iptal edilmiştir. Sisteme giriş yapabilmek için aboneliğinizin aktif olması gerekmektedir.',
            );
          }
  ...
  private async generateTokens(
    ...
    const effectivePermissions =
      permissions.length > 0
        ? permissions
        : await this.getUserPermissionKeys(userId);
    ...
    return { accessToken, refreshToken, permissions: effectivePermissions };
  }
  ...
  private async getUserPermissionKeys(userId: string): Promise<string[]> {
    ...
    if (
      ...
      (user.roleRelation?.isSystemRole &&
        user.roleRelation?.name === 'Yönetici')
    ) {
      return ['ALL'];
    }

    return (
      user.roleRelation?.permissions.map(
