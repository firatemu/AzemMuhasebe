...
if [[ ! -f "${OUT_DIR}/muhasebe-backend-staging.tar" ]] || [[ ! -f "${OUT_DIR}/muhasebe-user-panel-staging.tar" ]]; then
  echo "Hata: Önce lokal build yapın (Docker kurulu makinede): ./scripts/build-staging-local.sh"
  exit 1
fi

REMOTE_DIR="/var/www/muhasebe/dist-staging-images"
echo "=============================================="
...
scp "${OUT_DIR}/muhasebe-backend-staging.tar" "${OUT_DIR}/muhasebe-user-panel-staging.tar" "${REMOTE}:${REMOTE_DIR}/"

echo ""
echo "[3/3] Sunucuda docker load..."
ssh "$REMOTE" "cd $REMOTE_DIR && docker load -i muhasebe-backend-staging.tar && docker load -i muhasebe-user-panel-staging.tar"
