...
if docker exec muhasebe-minio mc ls myminio/muhasebe > /dev/null 2>&1; then
    echo "   ✅ Bucket 'muhasebe' exists"
else
...
VERSION_STATUS=$(docker exec muhasebe-minio mc version info myminio/muhasebe 2>&1 | grep "enabled" || echo "disabled")
...
if docker cp "$TEST_FILE" muhasebe-minio:/tmp/test.txt && \
   docker exec muhasebe-minio mc cp /tmp/test.txt myminio/muhasebe/test/ > /dev/null 2>&1; then
...
if docker exec muhasebe-minio mc ls myminio/muhasebe/test/ > /dev/null 2>&1; then
...
if docker exec muhasebe-minio mc rm myminio/muhasebe/test/test.txt > /dev/null 2>&1; then
...
# Cleanup
rm -f "$TEST_FILE"
docker exec muhasebe-minio rm -f /tmp/test.txt 2>/dev/null || true
