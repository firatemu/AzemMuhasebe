...
docker exec muhasebe-minio mc alias set backup http://localhost:9000 minioadmin minioadmin123 --api S3v4 2>/dev/null || true

# Create temp directory inside container
docker exec muhasebe-minio mkdir -p /tmp/backup-$DATE

# Mirror bucket to /tmp inside container
docker exec muhasebe-minio mc mirror --overwrite myminio/muhasebe /tmp/backup-$DATE/

# Copy backup from container to host
docker cp muhasebe-minio:/tmp/backup-$DATE "$BACKUP_PATH/muhasebe"

# Cleanup temp backup in container
docker exec muhasebe-minio rm -rf /tmp/backup-$DATE
